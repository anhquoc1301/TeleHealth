...
def get_mask_alung(vol):
    ...
    for idx in range(shp[0]):
            ...
            around_lung[idx, :, :] = binary_erosion(vol_im[idx], structure=np.ones((2,2))).astype(vol_im.dtype)
            around_lung[idx, :, :] = binary_fill_holes(around_lung[idx, :, :], structure=np.ones((6,6))).astype(vol_im.dtype)
            around_lung[idx, :, :] = binary_erosion( around_lung[idx, :, :], structure=np.ones((7,7))).astype(vol_im.dtype)
            around_lung[idx, :, :] = morphology.remove_small_objects(around_lung[idx, :, :], min_size=750)
            around_lung[idx, :, :] = binary_dilation( around_lung[idx, :, :], structure=np.ones((10,10))).astype(vol_im.dtype)
    return around_lung
...
def load_data(url, user, urlk):
  ...
  imgs_after_resamp, spacing = resample(imgs_to_process, patient, [1,r*1.99,c*1.99])
